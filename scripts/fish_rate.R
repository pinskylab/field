...
# for 2017, every fish obs was recorded as an anem obs with the dive type C
library(tidyverse)
library(stringr)
library(lubridate)
...
anem$obs_time <- lubridate::hms(anem$obs_time)
anem$obs_time <- lubridate::as_date(anem$obs_time)

anem$pause <- NA
# calculate the time between observations
for(i in 2:nrow(anem)){
  anem$pause[i] <- lubridate::interval(anem$obs_time[i-1], anem$obs_time[i])
}

anem <- anem %>% 
  mutate(pause = pause/60) %>% 
  select(dive_table_id, pause, site)
