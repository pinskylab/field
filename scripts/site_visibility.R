...
Mags <- left_join(Mag_dives, Mag_anems, by="dive_table_id") #put dives db first rather than anems b/c some of the dives don't have any anems associated with them

# Pull out the first non-NA anem_id seen on each dive (group by year first b/c dive_num gets reset each year)
Mag_repanem <- Mags %>% 
  mutate(dive_num =  as.numeric(dive_num)) %>% #make dive_num numeric rather than a character (don't mutate dive_num to be integer b/c not all are integers (there's a 11.1 and a 14.5))
...
#in 2013, dive_num 25 has no anems with numbers but also no visibility recorded so just put NA
...
# Rename original site column so can make a new site column
dives <- rename(dives, org_site = site) %>%
  mutate(site = rep(NA, length(dive_table_id))) 
...
for(i in 1:length(dives$dive_table_id)) {
  ...
  for(j in 1:length(Mag_df$year)){
    ...
    if(match !=0){
      dives$site[i] = Mag_df$site[match] #if there's a match, put in the site from the Mag_df match
    } else {
      dives$site[i] = dives$org_site[i] #otherwise, just put in the site originally in the dives df
    }
  }
}
...
# Make a dataframe of the visibility to add in for years when too poor of visibility to dive (Mag N in 2017 and 2017, Mag S in 2017)
...
# Add additional Mag info into the rest
dives_vis <- rbind(dives_visC, Mag2add)

# Find average vis per year by site, not averaging over day first (just averaging all dives), includes Mag N-S separation
dives_Avg <- dives_vis %>%
  group_by(year, site) %>%
  summarise(avg_vis = mean(visibility_m_clean, na.rm = TRUE))

# Replace Cab vis in 2017 (currently NA b/c no full sampling dives there) (can do it this way b/c already a Cab 2017 entry, just has NA vis)
dives_Avg <- dives_Avg %>%
